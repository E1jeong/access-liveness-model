...
def _make_interpreter(model_path):
    # 먼저 기본(XNNPACK) 경로로 시도해 속도를 확보하고, int8 그래프에서 XNNPACK이
    # prepare 실패하면 기본 delegate 없이(reference 커널) 재시도한다.
    # num_threads로 conv 등 멀티스레딩을 활성화해 reference 경로도 가속한다.
...
def _quantize_input(arr, detail):
    """정규화된 float32 입력을 모델 입력 dtype에 맞게 변환한다."""
    dtype = detail['dtype']
    if dtype == np.float32:
...
def evaluate(model_path, data_dir, folds, fold_idx, seed, max_samples=None):
    ...
    from train import calculate_validation_metrics
...
def main():
    ...
    parser.add_argument("--fold-idx", type=int, default=0, help="검증에 사용할 fold (변환에 쓴 fold와 일치시킬 것)")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--max-samples", type=int, default=None, help="빠른 확인용: 평가 샘플 수 제한")
