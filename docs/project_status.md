...
- `[검증 완료]` 현재 validation 317장의 confusion matrix는 `[[100, 0], [0, 217]]`, Accuracy는 100%다.
...
| 일시 | 변경 내용 | 근거 |
|---|---|---|
| 2026-06-22 14:11 | 초기 코드·환경·데이터·체크포인트·ONNX 상태 검증 | 저장소 정적 검사 및 로컬 실행 (Commit bd8159f) |
| 2026-06-22 15:22 | 고정 개발 가이드와 변경 상태 문서 분리 | AI 자율 개발 문서 개편 (Commit eba3d8d) |
| 2026-06-22 15:48 | 컬러 프린트 사진(Spoof) 데이터 수집 및 학습 적용 | 추가 데이터(총 376장)에 대한 전처리, 학습, ONNX 변환 및 검증 완료 |
| 2026-06-22 16:12 | 얼굴 검출 ROI 전처리 도입 및 수집 피드백(미러/프리뷰) 추가 | OpenCV Haar Cascade 얼굴 검출 연동, 실시간 바운딩 박스 가이드, 촬영 시 우측 하단 크롭본 프리뷰 오버레이 및 데이터셋 초기화 스크립트 구현 완료 |
| 2026-06-22 16:50 | 데이터셋 얼굴 크롭 누락 오류 수정 및 전체 재학습 | 신규 870장 데이터셋에 대해 얼굴 영역 크롭 전처리 적용, 학습(Train/Val Acc 100%) 및 ONNX 변환 완료하여 실시간 오분류 차단 |
