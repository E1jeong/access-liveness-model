...
- `docs/keras-concept-review.md` is the user's own comprehension record: its answers and every `현재 이해 상태` line state what the user has understood. Explain concepts in conversation; write into that file only when the user asks, and never fill in an understanding-state line on their behalf.
