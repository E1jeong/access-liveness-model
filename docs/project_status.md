...
| 2026-06-29 | [미검증] batch_size 증가 시 학습률 비례 증가 테스트 필요. 현재 batch_size=8, lr=1e-4 기준으로 batch_size=32면 lr=4e-4, batch_size=64면 lr=8e-4 시도 예정. GPU 활용률이 batch_size=8 기준 ~30% 수준으로 낮아 batch_size=32 이상이 학습 속도와 GPU 효율 모두 개선할 것으로 예상. |
