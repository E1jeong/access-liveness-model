...
def _rgb_imagenet_norm_to_mobilenet_range(rgb):
    """Convert ImageNet-normalized RGB to the export model's [-1, 1] range."""
    raw_0_1 = rgb * RGB_STD + RGB_MEAN
    return raw_0_1 * 2.0 - 1.0


def _copy_nested_weights(source_model, target_model, layer_name):
    """Copy every weighted layer in a nested backbone with strict checks."""
...
def _npu_parity_inputs(sample, model_type):
    """Build equivalent inputs for the training and NPU export models."""
    def batch(value):
...
def validate_npu_export_parity(trained_model, export_model, sample, model_type):
    """Reject an NPU export graph when it does not preserve Keras logits."""
...
def build_npu_export_model(trained_model, model_type):
    """Rebuild a trained model with the graph operations required by the NPU."""
    try:
    ...
    if "efficientnet_lite0" in trained_model.name or any(
        "efficientnet_lite0" in layer.name for layer in trained_model.layers
    ):
